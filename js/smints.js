	...
	$.fn.smint = function( options ) {
			...
			if (!scrollingDown) {
				while (true) {
					if (scrollTop >= optionLocs[curi].top) {
						$smintItems.parent().removeClass('activeMenu');
						$('#' + optionLocs[curi].id).parent().addClass('activeMenu');
		...
		$(window).scroll(function() {
			...
			if ($(window).scrollTop() + $(window).height() == $(document).height()) {
				$smintItems.parent().removeClass('activeMenu');
				$smintItems.last().parent().addClass('activeMenu');
			}
		});

		$smintItems.first().parent().addClass('activeMenu');
